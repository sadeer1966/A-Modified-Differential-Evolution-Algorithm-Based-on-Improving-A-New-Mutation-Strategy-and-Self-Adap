...
def DEalgorithm_classic(PopSize, fn, N, MaxItr, beta_min = 0, beta_max = 1, pCR = 0.25, lower = -30, upper = 30):
    besteval = float('inf')
    ...
    for k in range(MaxItr):
        for i in range(PopSize):
            ...
            # if np.mod(k, 2) == 0:
            #     pCR = np.random.uniform(0, 0.5)
            # else:
            #     pCR = np.random.uniform(0.5, 1)
